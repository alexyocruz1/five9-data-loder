...
const GeneralSkills = () => {
  const { apiRoute, setUsername, username } = useContext(AppContext);
  ...
  const handleExportData = () => {
    if (skills && skills.length > 0) {
      const csv = generateCsv(csvConfig)(skills);
      download(csvConfig)(csv);
    }
  };

  const fetchAllSkills = async (PassedUsername, PassedPassword, rememberUsername) => {
    setLoading(true);
    try {
      const response = await axios.post(`${apiRoute}/api/skills/getSkills/`, {
      ...
      setSkills(response.data.return);
      setUsername(rememberUsername ? PassedUsername : '');

      handleLoginSuccess('Credentials confirmed and skills data synced!');
    } catch (error) {
      const errorMsg = error.response?.data?.error?.message || error.message || 'Failed to fetch skills data';
  ...
  const columns = useMemo(() => {
    if (skills && skills.length > 0) {
      return Object.keys(skills[0]).map((key) => ({
        accessorKey: key,
        header: key.replace(/_/g, ' ').toUpperCase(),
        Cell: typeof skills[0][key] === 'boolean' ? booleanCellRenderer : undefined,
      }));
    }
    return [];
  }, [skills]);
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: ({ table }) => {
      const isDataAvailable = skills && skills.length > 0;
      return (
  ...
  return (
    <Container fluid={isTabletOrMobile}>
      <Row className="align-items-center mb-3">
        <Col>
          <div className="d-flex align-items-center">
            <h1 className="me-3 mb-0">General Skills</h1>
            <Button 
      ...
      >
        ...
        <Modal.Body>
          <Login username={username} endpoint={fetchAllSkills} loading={loading} />
        </Modal.Body>
      </Modal>
