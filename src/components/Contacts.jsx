...
const Contacts = () => {
  ...
  const fetchAllContacts = async (PassedUsername, PassedPassword, rememberUsername) => {
    ...
    try {
      ...
      handleLoginSuccess('Credentials confirmed and contacts synced!');
    } catch (error) {
      const errorMsg = error.response?.data?.error?.message || error.message || 'Failed to fetch contacts';
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: ({ table }) => {
      const isDataAvailable = contacts && contacts.length > 0;
      return (
      ...
      <Row className="align-items-center mb-3">
        <Col>
          <div className="d-flex align-items-center">
            <h1 className="me-3 mb-0">Contacts</h1>
            <Button 
